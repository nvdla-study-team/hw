// -----------------------------------------------------------------------------
// sdp2pdp_source_stub : sdp2pdp 驱动 stub（阶段4 前置；PDP UT 扮演 sdp 发送侧）
//   - send_beat(tr) task：valid 拉高保持至 ready 握手，握手后按 gap 旋钮插空拍
//     （连续调用且 gap=0 时背靠背）
//   - gap 旋钮：gap_min/gap_max（uvm_config_db int unsigned 可调，默认 0）
//   - 复位期间 valid 压 0
//   virtual interface 经 config_db 键 "sdp2pdp_vif"（多实例靠作用域区分）
// -----------------------------------------------------------------------------
`ifndef SDP2PDP_SOURCE_STUB_SVH
`define SDP2PDP_SOURCE_STUB_SVH

class sdp2pdp_source_stub extends uvm_component;
  ...
  `uvm_component_utils(sdp2pdp_source_stub)
  ...
  task send_beat(sdp2pdp_item tr);
    if (vif == null)
...
endclass : sdp2pdp_source_stub

`endif // SDP2PDP_SOURCE_STUB_SVH
