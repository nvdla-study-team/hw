...
// ----------------------------------------------------------------
// 【机制总览】两路加权轮转仲裁（WT 内 WMB vs WT 数据用）
//
// 权重语义：wt0/wt1（5bit，来自 S_ARBITER）是"连胜配额"——某路拿到
// 授权后装载 wt−1 到 wt_left，只要它持续请求且配额未耗尽（wt_left≠0）
// 就连续保持授权（每拍递减）；配额耗尽或该路空闲才重新选边。
// 换边规则：优先给上一持有者的对方（wrr_gnt=01 时先看 req[1]，反之
// 亦然；复位后 00 态先看 req[0]）——避免高权重路饿死对方。
// 权重为 0 的路请求被直接屏蔽（req 与 |wt 相与），即 0 = 禁用该路。
// gnt 输出为组合信号且被 gnt_busy 整体压掉（上层 back-package 暂存
// 占用出口时不发新授权）；单拍单授权由 one-hot 断言保证。
// ----------------------------------------------------------------
