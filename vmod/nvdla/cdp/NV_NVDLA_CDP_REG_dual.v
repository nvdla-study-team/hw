...
// ----------------------------------------------------------------
// 【机制块注】CDP dual 寄存器组（块内偏移 0x048~0x0b8，上层例化两份=d0/d1）
//
// 一份完整的"单层执行配置"：
//   - D_OP_ENABLE 0x048：乒乓分界后的第一个寄存器，写它产生
//     op_en_trigger 脉冲——真正的 op_en 状态位（含武装后锁写、完成
//     自动清零）在上层 CDP_reg 维护，本模块只报告写事件；
//   - 目的地址/步长 0x050~0x064（含 dst_ram_type：1=MC / 0=CV）；
//   - 数据格式与 datin/datout 换算参数 0x068~0x088、LRN 配置 0x070；
//   - NaN/Inf 统计与性能计数 0x08c~0x0b4：只读快照，值由上层反灌；
//   - D_CYA 0x0b8。
// 乒乓机制对本模块透明：写使能由上层按 producer 指针二选一给出，
// 数据通路取哪份由上层按 consumer 指针 mux，这里只是一份普通规整的
// 写译码 + 读 case 寄存器堆。
// ----------------------------------------------------------------
